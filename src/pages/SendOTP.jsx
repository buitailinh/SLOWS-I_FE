...
function SendOTP({ status = false}) {
  ...
  <path d="M487.9 207.4C489.7 206.8 490.3 208.8 492.1 208.2C493.9 207.6 493.3 205.7 495.1 205.2C496.9 204.6 497.5 206.6 499.3 206C501.1 205.4 500.5 203.5 502.3 203C504.1 202.5 504.7 204.4 506.5 203.8" stroke="#9E3FB7" stroke-width="2" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"/>
      ...
      <OTPInputContainer>
      { !isSend ? (
      <OTPForm onSubmit={handleSubmit1}>
      <div className="brand">
      ...
      <div className='mb-6'>
        ...
        <label className='absolute top-[-12px] left-2 text-slate-50 text-[1rem] font-medium bg-[#341559]' htmlFor='email'>Email/Phone</label>
        <input
          type="text"
      ...
      <button type="submit" className="button-submit mt-10">Send OTP</button>
      <button onClick={handleGoBack} className=" bg-gray-400 p-2 rounded-md mt-4">Go Back </button>
      </OTPForm>

      ): (
        <OTPForm onSubmit={handleSubmit2}>
        <div className="brand">
              <img src={Logo} alt="logo" />
              <h1>snappy</h1>
        </div>
      ...
      <button type="submit" className="button-submit mt-10">Confirm</button>
        <button type="submit" className=" bg-gray-400 p-2 rounded-md mt-4">Back to</button>
        </OTPForm>
      ...
      <div className=" text-slate-200 text-xs transform translate-y-[-60px]">Privacy policy and terms of service  <a to="/register">in here.</a></div>
      </FormContainer>
    <ToastContainer />
    </>
    );
