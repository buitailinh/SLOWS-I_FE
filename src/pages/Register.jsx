...
export default function Register() {
    ...
    const handleSubmit = async (event) =>{
        ...
        if(bandleValidation()){
            const { firstName, lastName, password, email, phone, brithday} = values;
         const data =   await axios.post(registerRoute, {
            ...
            console.log('data:',data)
              ...
              navigate('/sendMail', { state: { email } });
           
        }
    };

    const bandleValidation = () =>{
        const { password, confirmPassword, firstName, lastName, email, phone} = values;
        if(password !== confirmPassword){
            toast.error("password and confirm password should be same",toastOptions );
            return false;
        } else if (firstName.length < 2) {
          toast.error(
            "Firstname should be greater than 2 characters.",
            toastOptions
          );
          return false;
        } else if (lastName.length < 2) {
          toast.error(
            "Lastname should be greater than 2 characters.",
            toastOptions
          );
          return false;
        } 
    ...
    const handleGoogleLogin = async()=>  window.location.href = loginGGRoute;
  return (
    <>
      <FormContainer>
  ...
  <path d="M487.9 207.4C489.7 206.8 490.3 208.8 492.1 208.2C493.9 207.6 493.3 205.7 495.1 205.2C496.9 204.6 497.5 206.6 499.3 206C501.1 205.4 500.5 203.5 502.3 203C504.1 202.5 504.7 204.4 506.5 203.8" stroke="#9E3FB7" stroke-width="2" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"/>
        ...
        <form action="" onSubmit={(event) => handleSubmit(event)}>
          ...
          <div className='relative'>
            <label className='absolute top-[-12px] left-2 text-slate-50 text-[1rem] font-medium bg-[#341559]' htmlFor='firstName'>First Name</label>
            <input
            type="text"
            placeholder="First name (*)"
            className='w-full pl-8 pr-3 py-2 rounded-md border border-gray-400 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-purple-600 focus:border-transparent'
            name="firstName"
            onChange={(e) => handleChange(e)}
          />
          </div>
          <div className='relative'>
          <label className='absolute top-[-12px] left-2 text-slate-50 text-[1rem] font-medium bg-[#341559]' htmlFor='lastName'>Last Name</label>
            <input
            type="text"
          ...
          <div  className='grid grid-cols-2 gap-4'>
          <div className='relative '>
          <label className='absolute top-[-12px] left-2 text-slate-50 text-[1rem] font-medium bg-[#341559]' htmlFor='firstName'>Phone</label>
            <input
            type="text"
          ...
          <div className='relative'>
          <label className='absolute top-[-12px] left-2 text-slate-50 text-[1rem] font-medium bg-[#341559]' htmlFor='firstName'>Brithday</label>
            <input
            type="date"
        ...
        <div className='relative'>
          <label className='absolute top-[-12px] left-2 text-slate-50 text-[1rem] font-medium bg-[#341559]' htmlFor='email'>Comfirm Password</label>
          <input
            type="password"
          ...
          <button type="submit" className='buttom-submit'>Create User</button>
